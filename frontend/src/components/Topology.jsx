...
export default function Topology({ elements, onSelectHost }) {
  ...
  useEffect(() => {
    ...
    const nodes = (elements?.nodes || []).map((n) => {
      ...
      const badge = BADGE[n.data.type] || '?';
      const raw = baseLabel || n.data.ip || '';
      const short = raw.length > 18 ? `${raw.slice(0, 17)}…` : raw;
      return {
        ...n,
        data: {
          ...n.data,
          display_label: `${badge}\u2002${short.replace(/\n/g, ' ')}`,
    ...
    const cy = cytoscape({
      ...
      style: [
        ...
        {
          // Device type is shown as a clean letter badge inside the label
          // (cytoscape background-image SVG data-URIs clip/garble in this
          // environment, so we use a text badge prefix instead).
          selector: 'node',
          style: {
            'font-weight': 600,
